use axum::extract::{State, WebSocketUpgrade, Json, Path};
...
use crate::responses::{AuroriteErrorResponse, FailableResponse, MessageInfo, SessionClientInfo, SessionInfo};
...
async fn post_message(
    ...
    tokio::spawn( async move {
        state.manager.session(id)
            .unwrap()
            .value()
            .broadcast(WebsocketMessage::Chat {content: body.content, created_at: now, client: info })
            .await
    });
