...
impl TryFrom<&Campaign> for CampaignInfo {
    ...
    fn try_from(value: &Campaign) -> Result<Self, Self::Error> {
        ...
        let masters: Vec<ClientInfo> = value.clients.get()
            .iter().filter(|cl| cl.is_master)
            .map(|cl| ClientInfo::from(cl.client.get())).collect();
        let players = value.clients.get()
            .iter().filter(|cl| !cl.is_master)
            .map(|cl| ClientInfo::from(cl.client.get())).collect();

        Ok(Self {
            masters, players, id: value.id, title: value.title.clone()
        })
    }
}